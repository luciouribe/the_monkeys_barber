...
<q-page padding>
    <div class="q-pa-md">
        <q-table title="Unidad de medida" :rows="rows" :columns="columns" row-key="id" :filter="filter">
            <template v-slot:top-right>
                <q-btn flat color="green-6" icon-right="add" label="Crear" @click="go_tonew" v-show="setVisible('inventario.uom.create')" />
...
export default {
    ...
    computed: {
        getSolicitudes() {
            return this.$store.getters["productos/getterUOM"];
        },
    },
    methods: {
        ...mapActions("productos", ["fetchUOM"]),
        setVisible(permiso) {
