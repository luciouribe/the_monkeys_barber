...
<q-page padding>
    <div class="q-pa-md">
        <q-table title="Productos" :rows="rows" :columns="columns" row-key="id" :filter="filter">
            <template v-slot:top-right>
                <q-btn flat color="green-6" icon-right="add" label="Crear" @click="go_tonew" v-show="setVisible('pos.productos.create')"/>
            ...
            <template v-slot:body="props">
                <q-tr clickable :props="props" @click="go_todetalle(props.row.id)">
                    ...
                    <q-td key="precio" :props="props">
                        <q-badge outline color="secondary" :label="`$ ${props.row.precio}`" />
                    </q-td>
                    <q-td key="existencias" :props="props">
                        <q-badge outline color="primary" :label="`${props.row.existencias}`" />
                    </q-td>
                </q-tr>
            </template>
        </q-table>
...
export default {
    ...
    methods: {
        ...
        getProductos() {
            this.fetchProductos().then((response) => {
                this.rows = response;
            });
        },
        go_todetalle(id) {
